...
def TeleportDynamicPhysicObject(n, shift):
    ...
    hl.gVal.physics.NodeResetWorld(n, hl.TranslationMat4(hl.Vec3(pos.x, pos.y, pos.z) + shift), False)


# if out of grid, respawn on the other side
def Teleport():
    global objectHasBeenTeleportShift, worldSimulatedCenter
    if pos.x > 25:
        objectHasBeenTeleportShift = hl.Vec3(-gridSize.x, 0, 0)
        TeleportDynamicPhysicObject(nodeSphere, objectHasBeenTeleportShift)
    elif pos.z > 25:
        objectHasBeenTeleportShift = hl.Vec3(0, 0, -gridSize.x)
        TeleportDynamicPhysicObject(nodeSphere, objectHasBeenTeleportShift)
    elif pos.x < -25:
        objectHasBeenTeleportShift = hl.Vec3(gridSize.x, 0, 0)
        TeleportDynamicPhysicObject(nodeSphere, objectHasBeenTeleportShift)
    ...
    elif objectHasBeenTeleportShift is not None:
        # update world center
